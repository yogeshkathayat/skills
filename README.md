...
| [ship-playbook](#ship-playbook) | Prompt → planned, built, reviewed, audited — chains plan/review/build/audit skills as one configurable Workflow (one pass, returns findings); slice-scoped per-task review + a live `.ulpi/workflows/<id>.json` status file (status/stop/resume) |
